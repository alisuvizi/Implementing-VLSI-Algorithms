/**
	*********************************************************
	In this file, basic functions of parser are implemented.
	*********************************************************
**/

#include <stdlib.h>
#include <string.h>
#include <placer.h>
#include <stdio.h>
#include <math.h>
#include <iostream>
#include <list>
#include <vector>
#include <map>
#include <atlasDB.h>

#include <algorithm>


using namespace std;
unsigned layoutH, layoutW, cellHeight;

#define SRC 101
#define SNK 102

#define __whiteSpace 1.1

#define __whiteSpace 1.1
#define DBU 2000

//-----------------------------------------------------------
// Constructors
//-----------------------------------------------------------
SimplePlacer::SimplePlacer()
{
	mList = new list <simpleInstance *>;
	nList = new list <simpleNet *>;
}

//-----------------------------------------------------------
// Destructors
//-----------------------------------------------------------
SimplePlacer::~SimplePlacer()
{
}

//-----------------------------------------------------------
// Read DB information
//-----------------------------------------------------------
int SimplePlacer::readDB ( atlasDB *db )
{
	TInstData *dbInst = new TInstData;
	list <simpleInstance *>::iterator iter;
	simpleInstance *anInst;
	TNetData *dbNet = new TNetData;
	TPinData *dbPin = new TPinData;
	TPointData *pinXY = new TPointData;
	float layoutArea = 0, Dim, modDim;
	int cnt;
	printf ( "\tReading %d cells ... \n", db->getInstancesNum() );
	//------------------
	// Reading Instances
	//------------------
	db->resetInstListPos();
	cnt = 0;
	while ( db->getInstData ( dbInst ) == true )
	{
		//--------------------------
		// Creating a new instance
		//--------------------------
		anInst = new simpleInstance;
		anInst->rowNo = 0;
		anInst -> TL.x = 0;
		anInst -> TL.y = 0;
		anInst -> InstID = cnt + 1;
		anInst -> InstName = new char[100];
		strcpy ( anInst -> InstName, dbInst->instName.c_str() );
		anInst -> CellName = new char[100];
		strcpy ( anInst -> CellName, dbInst->macroName.c_str() );
		anInst -> Width = dbInst->width;
		anInst -> Height = dbInst->height;
		rowHeight = anInst->Height;
		mList->push_back ( anInst );
		db->instGoForward();
		cnt ++;
	}
	cellHeight = rowHeight;

	//------------------
	// Creating a new net
	//------------------
	printf ( "\tReading and elaborating %d nets ... ", db->getNetsNum() );
	unsigned nn = 0;
	db->resetNetListPos();
	while ( db->getNetData ( dbNet ) == true )
	{
		simpleNet *aNet;
		aNet = new simpleNet;
		aNet -> netName = new char[100];
		aNet->instances = new list <struct simpleInstance *>;
		aNet->srcTerminal = NULL;
		strcpy ( aNet -> netName, dbNet -> name . c_str() );

		nn ++;
		if ( dbNet -> name == "POWR" )
		{
			printf ( "\t\tNet Power is ignored.\n" );
			db->NetDataGoForward();
			continue;
		}
		if ( dbNet -> name == "GRND" )
		{
			printf ( "\t\tNet Ground is ignored.\n" );
			db->NetDataGoForward();
			continue;
		}
		db->resetPinOfNetListPos();
		while ( db->getPinOfNetData ( dbPin ) == true )
		{
			db->getPinParentPos ( pinXY );
			db->getPinParent ( dbInst );
			simpleInstance *anInst =  findInstance ( dbInst->instName );
			if ( anInst == NULL )
			{
				printf ( "Netlist annotation error.\n" );
			}
			else
			{
				aNet->instances->push_back ( anInst );
				if ( dbPin->dir == output )
					aNet->srcTerminal = anInst;
			}
			db->PinOfNetGoForward();
		}
		

		aNet -> fixedPoint = findfixedPoint(aNet -> netName);
		
		nList->push_back ( aNet );
		db->NetDataGoForward();
	}
	

	//----------------------------
	// Estimate layout dimension
	//----------------------------
	for ( iter = mList->begin(); iter != mList->end(); iter ++ )
	{
		anInst = ( simpleInstance * ) ( *iter );
		layoutArea += anInst->Width * anInst->Height;
	}

	Dim = floor ( sqrt ( layoutArea * __whiteSpace ) );
	modDim = Dim - ( int ) Dim % ( int ) rowHeight;
	maxlayoutW = ( unsigned ) ( modDim );
	maxlayoutH = ( unsigned ) ( modDim );
	printf ( "\tRead DB: %d insts are read.\n", mList->size() );
	printf ( "\tRead DB: %d nets are read.\n" , nList->size() );
	

	mSize = db->getInstancesNum(); 

	return 0;
}

//-----------------------------------
// Read Placed Netlist from file
//-----------------------------------
void SimplePlacer::readDump(char *dumpFile)
{
	FILE *fp;
	char iName[64];
	float iLeft, iTop;
	unsigned int cnt = 0, iNumber = 0; 
	
	fp = fopen(dumpFile, "r+t");
	if (fp == NULL)
	{
	    printf("Cannot read dump file %s\n", dumpFile);
	    exit(1);
	}

	fscanf(fp,"%d %d %d %d\n", &iNumber, &maxlayoutW, &maxlayoutH, &rowNumber);


	for(unsigned i = 0;i < iNumber;i ++)
	{
	    fscanf(fp,"%s %f %f \n",iName, &iLeft, &iTop);

	    simpleInstance *iPtr = findInstance(iName);
	    if (iPtr == NULL)
	    {
		printf("Instance %s is not found in instance list.\n");
		exit(1);
	    }
	    else
	    {
		iPtr -> TL.x = iLeft;
		iPtr -> TL.y = iTop;
		cnt ++;
	    }
	}
	fixedPointCnt = cnt;
	printf("%d in stances are loaded from dump\n", cnt);
}

simpleInstance *SimplePlacer::findInstance ( string instName )
{
	list <simpleInstance *>::iterator iter;
	simpleInstance *anInst;

	for ( iter = mList->begin(); iter != mList->end(); iter ++ )
	{
		anInst = ( simpleInstance * ) ( * iter );
		if ( strcmp ( anInst-> InstName, instName.c_str() ) == 0 )
			return anInst;
	}
	return NULL;
}

int SimplePlacer::simPlacer()
{
	list <simpleInstance *>::iterator iter;
	simpleInstance *anInst;
	int rowNo, i;
	float rowWidth;

	fixedSet();
	
	createMatrix();
	
	
	printf("Please Wait!\n");
	LU_Solver();
	
	
	newInst();
	
	//Legalize
	mList->sort(simpleInst_cmp());

	rowNo = 0;
	rowWidth = 0;
	for ( iter = mList->begin(); iter != mList->end(); iter ++ )
	{
		anInst = ( simpleInstance * ) ( * iter );
		if ( rowWidth < maxlayoutW )
		{
			anInst->rowNo = rowNo;
			anInst->TL.x = rowWidth;
			anInst->TL.y = rowNo * rowHeight;
			rowWidth += anInst->Width;
		}
		else
		{
			rowNo ++;
			anInst->rowNo = rowNo;
			anInst->TL.x = 0;
			anInst->TL.y = rowNo * rowHeight;
			rowWidth = anInst->Width;;
		}
		
	}
	rowNumber = rowNo;
	layoutW = maxlayoutW;
	layoutH = maxlayoutH;
	return 1;
}

void SimplePlacer::updateDB ( atlasDB *db )
{
	list <simpleInstance *>::iterator iter;
	simpleInstance *iPtr;
	unsigned maxX = 0, maxY = 0;;
	int i;
	int totalCellNo = 0;

	for ( iter = mList->begin(); iter != mList->end(); iter ++ )
	{
		iPtr = ( simpleInstance * ) ( *iter );

		if ( db->setInstTopLeft ( iPtr->InstName, ( long int ) ( iPtr->TL.y ), ( long int ) ( iPtr->TL.x ) ) == false )
		{
			printf ( "Update DB violation!\n" );
			exit ( 1 );
		}
		totalCellNo ++;
		if ( ( iPtr->TL.x + iPtr->Width ) > maxX )
			maxX = iPtr->TL.x + iPtr->Width;

		if ( ( iPtr->TL.y + iPtr->Height ) > maxY )
			maxY = iPtr->TL.y + iPtr->Height;

		maxlayoutW = maxX;
		maxlayoutH = maxY;
	}

	printf ( "\tUpdate: %d instances are updated in DB\n", totalCellNo );
	printf ( "\tUpdate: layout area: w = %d , h = %d\n", maxlayoutW, maxlayoutH );

	//	set layout dimensions
	db->setNetlistPlaceResults ( ( long int ) ( maxX ), ( long int ) ( maxY ), rowNumber, ( long int ) ( rowHeight ) );
}

void SimplePlacer::dumpPlacement ( char *dumpFile )
{
	list <simpleInstance *>::iterator iter1;
	simpleInstance *anInst;
	FILE *fp;
    

	fp = fopen(dumpFile, "w+t");
	if (fp == NULL)
	{
	    printf("Cannot write dump file %s\n", dumpFile);
	    exit(1);
	}
	fprintf(fp,"%d %d %d %d\n",mList->size(), (unsigned int) (maxlayoutW), (unsigned int)(maxlayoutH), rowNumber);

	for ( iter1 = mList->begin();iter1 != mList->end();iter1 ++ )
	{
	    anInst = ( simpleInstance * ) ( * iter1 );
	    fprintf(fp,"%s %f %f\n", anInst->InstName, (float)(anInst->TL.x), (float)(anInst->TL.y) );
	}
	fclose(fp);
}


double SimplePlacer::THPWL()
{
	list <simpleNet *>::iterator iter1;
	list <simpleInstance *>::iterator iter2;
	simpleNet *aNet;
	simpleInstance *anInst;
	long int wl, minx, miny, maxx, maxy;
	double THPWL = 0;

	for ( iter1 = nList->begin();iter1 != nList->end();iter1 ++ )
	{
		aNet = ( simpleNet * ) ( * iter1 );
		wl = 0;
		minx = 0x7FFFFFFF;
		miny = 0x7FFFFFFF;
		maxx = -1;
		maxy = -1;
		for ( iter2 = aNet->instances->begin();iter2 != aNet->instances->end();iter2 ++ )
		{
			anInst = ( simpleInstance * ) ( * iter2 );
			if ( anInst->TL.x < minx )
				minx = anInst->TL.x;
			if ( anInst->TL.x > maxx )
				maxx = anInst->TL.x;
			if ( anInst->TL.y < miny )
				miny = anInst->TL.y;
			if ( anInst->TL.y > maxy )
				maxy = anInst->TL.y;
			wl += ( maxy - miny ) + ( maxx - minx );
		}
		THPWL += wl;
	}
	THPWL /= DBU;
	//printf("\t\tTotal half perimeter wirelength = %lf um\n", THPWL);
	return THPWL;
}

//-----------------------------------------------------------
//Find Fix point (IO Pin)
//-----------------------------------------------------------
simpleFixedPoint *SimplePlacer::findfixedPoint ( string pinName )
{
	list <simpleFixedPoint *>::iterator iter;
	simpleFixedPoint *aPin;

	for ( iter = fList->begin(); iter != fList->end(); iter ++ )
	{
		aPin = ( simpleFixedPoint * ) ( * iter );
		if ( strcmp ( aPin-> pinName, pinName.c_str() ) == 0 )
			return aPin;
	}
	return NULL;
}

//-----------------------------------------------------------
//Read Fix point From spi_FixedPoint.def
//-----------------------------------------------------------
void SimplePlacer::readfixedPoints(char *fixedPointsFile)
{
	FILE *fp;
    size_t len = 0;
    ssize_t read;
    char * line = NULL;

	unsigned int cnt = 0; 

	fList = new list <simpleFixedPoint *>;
	fp = fopen(fixedPointsFile, "r");
	if (fp == NULL)
	{
	    exit(1);
	}

    while ((read = getline(&line, &len, fp)) != -1) {
     
        if( (line[0]=='/') && (line[1]=='/') )
        {
        	continue;
        } 
	    if(line[read-1]=='\n')
	    {
	    	line[read-1]=0;
	    }
	    simpleFixedPoint *pPtr = new simpleFixedPoint;
	    pPtr->pinName = new char[20];
	    strcpy ( pPtr->pinName, line);
        
		pPtr-> TL.x = 0;
		pPtr-> TL.y = 0;
		cnt ++;
	    fList->push_back ( pPtr );
    }
    fixedPointCnt = cnt;
	
}
//-----------------------------------------------------------
// Place Fixed Point
//-----------------------------------------------------------
void SimplePlacer::fixedSet (  )
{
	list <simpleFixedPoint *>::iterator iter;
	simpleFixedPoint *pPtr;
	int PitchSizeX, PitchSizeY ;
	int FixedNo, cnt;
	int layoutW, layoutH;

	//printf("fixedPointCnt=%d\n", fixedPointCnt);
    	FixedNo = fixedPointCnt/4;
    	if(FixedNo*4<fixedPointCnt)
    	FixedNo++;

   	cnt=0;
	layoutW=maxlayoutW;
	layoutH=maxlayoutH;
	PitchSizeX = layoutW/FixedNo;
	PitchSizeY = layoutH/FixedNo;


	for ( iter = fList->begin(); iter != fList->end(); iter ++, cnt++ )
	{
		pPtr = ( simpleFixedPoint * ) ( * iter );
		if(cnt<FixedNo*1){
			//Left
			pPtr-> TL.x = 0;
			pPtr-> TL.y = cnt*PitchSizeY;
		} 
		else if(cnt<FixedNo*2){
			//Right
			pPtr-> TL.x = layoutW;
			pPtr-> TL.y = cnt*PitchSizeY;
		}
		else if(cnt<FixedNo*3){
			//Bottom
			pPtr-> TL.x = cnt*PitchSizeX;
			pPtr-> TL.y = 0;
		} 
		else if(cnt<FixedNo*4){
			//Top
			pPtr-> TL.x = cnt*PitchSizeX;
			pPtr-> TL.y = layoutH;
		}

		
	}

}

//-----------------------------------------------------------

void SimplePlacer::createMatrix (  )
{

	list <simpleNet *>::iterator iter1;
	list <simpleInstance *>::iterator iter2, iter3;
	simpleNet *aNet;
	simpleInstance *iInst, *jInst;
	long int i, j;
	long int mat_i, mat_j;

    
    mat =(float *) malloc(mSize * mSize * sizeof(float));
    Ax =(float *) malloc(mSize * sizeof(float));
	Ay =(float *) malloc(mSize * sizeof(float));

	for(i=0; i<mSize; i++)
	{
		for(j=0; j<mSize; j++)
		{
			*(mat + i*mSize + j) = 0.0;			
		}

		*(mat + i*mSize + i) = 2.0;//-->example
		*(Ax+i) = 0.0 ;
		*(Ay+i) = 0.0 ;
	}


	//Note
	//anInst -> InstID is started from 1
	//But matix is 0 started.
	//So -1 required.

	for ( iter1 = nList->begin();iter1 != nList->end();iter1 ++ )
	{
		aNet = ( simpleNet * ) ( * iter1 );
		for ( iter2 = aNet->instances->begin();iter2 != aNet->instances->end();iter2 ++ )
		{
			iInst = ( simpleInstance * ) ( * iter2 );
			mat_i = iInst-> InstID -1;
			if(aNet->fixedPoint)
			{
				*(Ax+mat_i) += aNet -> fixedPoint -> TL.x;
				*(Ay+mat_i) += aNet -> fixedPoint -> TL.y;
			}
			iter3 = iter2;
			for ( iter3 ++ ;iter3 != aNet->instances->end();iter3 ++ )
			{
				jInst = ( simpleInstance * ) ( * iter3 );
				mat_j = jInst-> InstID -1;


				if(mat_j!=mat_i)
				{
					*(mat+mat_i*mSize+mat_j) -= 0.5;
					*(mat+mat_j*mSize+mat_i) -= 0.5; 
				}
			}
		}
		
	}

}
//-----------------------------------------------------------
void SimplePlacer::LU_Solver (  )
{
	float c, c_inv ;
	float temp ;
	int i, j, k;
	//------------------------------------------
	//make Upper Matrix
	for(i=0; i<mSize; i++)
	{
		for(j=0; j<=i; j++)
		{
			if(i==j)
			{
			
				if( *(mat+i*mSize+j) == 0.0)
				{
					*(mat+i*mSize+j) = 1.0;
				}

				if(*(mat+i*mSize+j) != 1.0)
				{
					c = *(mat+i*mSize+j);
					c_inv= 1.0f / c;
					for(k=j; k<mSize; k++)
					{
						temp = *(mat+i*mSize+k);
						if(temp != 0.0)
							*(mat+i*mSize+k) = temp * c_inv;
					}
					*(Ax+i) *= c_inv;
					*(Ay+i) *= c_inv;
				}
			} 
			else
			{
				if(*(mat+i*mSize+j) != 0.0)
				{
					c = *(mat+i*mSize+j);
					for(k=j; k<mSize; k++)
					{
						temp = *(mat+j*mSize+k) ;
						if(temp != 0.0)
							*(mat+i*mSize+k) -= c * temp;
					}
					*(Ax+i) -= c * (*(Ax+j)) ;
					*(Ay+i) -= c * (*(Ay+j)) ;
				}
			}
		}
     
	}
	//------------------------------------------
	//make Lower(Diaginal) Matrix
	for(i=mSize-1; i>=0; i--)
	{
		for(j=i+1; j<mSize; j++)
		{
			if(i==j)
			{
				if(*(mat+i*mSize+j) != 1.0)
				{
					c = *(mat+i*mSize+j);
					for(k=j; k<mSize; k++)
					{
						*(mat+i*mSize+k) /= c;
					}
					*(Ax+i) /= c;
					*(Ay+i) /= c;
           		}
			} 
        	else
			{
				if(*(mat+i*mSize+j) != 0.0)
				{
						c = *(mat+i*mSize+j);
					for(k=j; k<mSize; k++)
					{
						*(mat+i*mSize+k) -= c * (*(mat+j*mSize+k));
					}
					*(Ax+i) -= c * (*(Ax+j));
					*(Ay+i) -= c * (*(Ay+j));
				}
			}
		}
	}
}

float Distance( simplePoint* pt1 , simplePoint* pt2)
{
	float dx, dy;
	dx = pt2->x - pt1->x;
	dy = pt2->y - pt1->y;
	
	return dx*dx + dy*dy;
}

double SimplePlacer::CostFunction()
{
	list <simpleNet *>::iterator iter1;
	list <simpleInstance *>::iterator iter2;
	list <simpleInstance *>::iterator iter3;
	simpleInstance *anInst2, *anInst3;
	simpleNet *aNet;

	long int wl, minx, miny, maxx, maxy;
	double cost = 0;
	simplePoint *pt1, *pt2;


	for ( iter1 = nList->begin();iter1 != nList->end();iter1 ++ )
	{
		aNet = ( simpleNet * ) ( * iter1 );
		for ( iter2 = aNet->instances->begin();iter2 != aNet->instances->end();iter2 ++ )
		{
			anInst2 = ( simpleInstance * ) ( * iter2 );

			for ( iter3 = aNet->instances->begin();iter3 != aNet->instances->end();iter3 ++ )
			{
				anInst3 = ( simpleInstance * ) ( * iter3 );
				
				cost += Distance(&anInst2->TL, &anInst3->TL);
			}
			
		}
	}

	cost = 0.5 * cost ;
	return cost;
}

//-----------------------------------------------------------

//-----------------------------------------------------------
void SimplePlacer::newInst (  )
{
	list <simpleInstance *>::iterator iter;
	simpleInstance *anInst;
	unsigned mat_i;

	for ( iter = mList->begin(); iter != mList->end(); iter ++ )
	{
		anInst = ( simpleInstance * ) ( * iter );
		mat_i = anInst-> InstID -1;
		anInst ->  x = *(Ax+mat_i);
		anInst ->  y = *(Ay+mat_i);
	}

}
//-----------------------------------------------------------
