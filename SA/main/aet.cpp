...
void usage(){
	cout << "Invalid innput; To use SA: -saplace <dumpfile> <T_0> <alpha> <Tmin> <inner_count>" << endl;
...
void runAlgorithms ( int argc, char **argv )
{
	if ( argc >= 5 )
	{
		...
		if (strcmp(argv[3],"-saplace") == 0)
		{
		    printf ( "3. SA placer and dump placed cells.\n" );
		    if (argc < 7){
		    	usage();
		    	exit(1);
		    }
		    saPL = new SimplePlacer::SimplePlacer(atof(argv[5]), atof(argv[6]), atof(argv[7]), atoi(argv[8]));
		    // Read information from DB
		    saPL->readDB ( db );
		    // Do Placement
		    saPL->SAplacer();
